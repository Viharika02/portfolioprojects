...
SELECT 
    ...
    -- Uncomment the following line to calculate the percentage of the population vaccinated
    -- ,(SUM(TRY_CAST(vac.new_vaccinations AS BIGINT)) OVER (PARTITION BY dea.location ORDER BY dea.date) / dea.population) * 100 AS VaccinationPercentage
FROM 
